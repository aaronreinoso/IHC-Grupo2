...
const TareaForm: React.FC = () => {
  ...
  const [touched, setTouched] = useState<Partial<Record<keyof TareaFormState, boolean>>>({});
    ...
    if (tareaId) {
      ...
      (async () => {
        ...
        if (data) {
          ...
          setFeedback("No pudimos cargar esta tarea. Intenta nuevamente en unos segundos.");
        }
      })();
    } else {
      setEditMode(false);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    const limit = TAREA_LIMITS[name as FieldName];
    ...
    if (errors[name as keyof TareaFormState]) {
      const fieldError = validateTareaField(name as keyof TareaFormState, finalValue);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (limiteMinutosPlan !== null) {
      ...
      if (requiredMinutes > limiteMinutosPlan) {
        setFeedback(`Advertencia: este plan dura ${limiteMinutosPlan} min. Para mantener una prueba realista, solo permite ${Math.floor(limiteMinutosPlan / 2)} tareas de 2 min cada una.`);
        setLoading(false);
        return;
      }
    }
    ...
    try {
      ...
      if (error) {
        setFeedback("No pudimos guardar la tarea. Revisa tu conexión e intenta nuevamente.");
      } else {
      ...
      setFeedback("Ocurrió un problema inesperado al guardar. Intenta nuevamente.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderFieldFeedback = (field: FieldName) => {
    ...
    if (status === "error") {
      ...
      message = `Sigue escribiendo: faltan ${min - length} caracteres para completar este campo.`;
    }
  ...
  const renderMotivationalMessage = () => {
    ...
    if (isFormComplete) {
      return (
        <div
          ...
          aria-live="polite"
        >
          ✅ ¡Excelente! La tarea cumple con los criterios mínimos de claridad, medición y contexto. Ya puedes guardarla.
        </div>
      );
    }
    ...
    return (
      <div
        ...
        aria-live="polite"
      >
        💡 Vas bien: completa o mejora {remainingFields} campo{remainingFields === 1 ? "" : "s"} para que la tarea sea clara, medible y útil en la prueba de usabilidad.
      </div>
    );
  };
